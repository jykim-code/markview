...
**Markview**는 마크다운(.md) 파일을 사람이 읽기 편한 형태로 즉시 렌더링하는 **웹 서비스**와 **크롬 확장 프로그램**의 통합 솔루션입니다.

- **웹 서비스**: `https://markview-4hy.pages.dev`
  - .md 파일을 업로드하면 고유 URL이 생성되고, 해당 URL만 공유하면 누구나 렌더링된 문서를 브라우저에서 즉시 열람
- **크롬 확장 프로그램**: Markview - Markdown Viewer (v1.0.0)
...
| 크롬 확장 프로그램 개발 | 별도 개발자 1인 | 개발자 1인이 수행 |
...
| 확장 표준 | Chrome Extension Manifest | V3 |
...
# 4. 배포
# Chrome Web Store 또는 chrome://extensions에서 dist/ 로드
```
...
| **Chrome Web Store 정식 배포** | 크롬 확장 프로그램 스토어 공개 배포 (현재 내부 배포) |
