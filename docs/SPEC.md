...
> 최신 상태는 **[`docs/ROADMAP.md`](./ROADMAP.md) §5**를 기준으로 본다. 아래 표는 2026-08-10에
> 맞춰 갱신했다.

| 항목 | 현황 |
|------|------|
| 인증 | 없음 — 누구나 업로드/편집 가능. 로그인은 미착수(인증 방식 미결, ROADMAP §2) |
| 문서 삭제 | 구현됨 (2026-07-30) — `DELETE /api/documents/[slug]`, `X-Owner-Token` 소유 증명 필요 |
| 문서 목록 | `/my` 구현됨 (2026-07-30) — 이 브라우저의 localStorage 기반. 기기 간 동기화는 계정 필요 |
| 되돌리기 | 직전 버전 1개 보관 구현됨 (2026-07-30) — R2 `_prev/{slug}`. 5MB 초과 문서는 미보관 |
| 파일 크기 | **MD·HTML 공통 25MB** (2026-07-22 상향, 이전 512KB/2MB) |
| 이미지 호스팅 | 미지원 — 외부 URL만 가능 |
| 문서 만료 | 없음 — 영구 보관 (2026-07-30 기본값으로 확정) |
| 접근 제어 | 없음 — URL을 아는 누구나 열람/편집. 삭제만 소유 증명 필요 |
...
- 사용자 인증 (내 문서 목록은 `/my`로 구현됨 — 계정은 기기 간 동기화용)
- 문서 만료 (삭제는 구현됨)
- 이미지 업로드 (R2 연동)
- 문서 비밀번호/비공개 설정
- 실시간 협업 편집
